...
class FaissRAGIndex:
    ...
    def _faiss():
        try:
            ...
            raise ImportError(
                "FAISS non è installato. "
                "Eseguire: pip install faiss-cpu"
            ) from exc
        return faiss

    @classmethod
    def _normalise_for_cosine(
        cls,
        vectors: np.ndarray,
    ) -> np.ndarray:
        faiss = cls._faiss()
    ...
    def fit(
        ...
        if set(np.unique(labels).tolist()) != {0, 1}:
            raise ValueError(
                "L'indice richiede entrambe le classi 0 e 1."
            )
        ...
        self.z_vectors = self.scaler.fit_transform(
            imputed
        ).astype(np.float32)

        retrieval_vectors = self._normalise_for_cosine(
            self.z_vectors
        )

        self.global_index = faiss.IndexFlatIP(
            retrieval_vectors.shape[1]
        )
    ...
    def _check_ready(self) -> None:
        if (
            ...
            raise RuntimeError(
                "Indice FAISS non costruito o non caricato."
            )
    ...
    def transform_query(
        ...
        z_query = self.scaler.transform(
            imputed
        ).astype(np.float32)
        retrieval_query = self._normalise_for_cosine(
            z_query
        )
    ...
    def majority_vote(
        ...
        row: pd.Series | np.ndarray,
        k: int,
    ) -> tuple[int, dict[int, int], list[Neighbor]]:
        neighbors = self.search(row, k)

        if not neighbors:
            raise RuntimeError(
                "FAISS non ha restituito vicini."
            )
    ...
    def _paths(
        ...
        return {
            "global": Path(
                str(base) + "_global.faiss"
            ),
            "meta": Path(
                str(base) + "_metadata.pkl"
            ),
        }
    ...
    def exists(
        ...
        return all(
            path.exists()
            for path in cls._paths(prefix).values()
        )
    ...
    def load(
        ...
        if not all(
            ...
            raise FileNotFoundError(
                f"Cache FAISS incompleta: {prefix!s}"
            )

        self.global_index = faiss.read_index(
            str(paths["global"])
        )
        ...
        self.cache_signature = metadata.get(
            "cache_signature"
        )